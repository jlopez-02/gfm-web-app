...
const generateGrafanaUrls = (theme, selectedId, totalHours) => {
  ...
  return {
    ...
    graph_url: `${host_url}/02-generacion?orgId=1&var-custom=mostrar&var-Instalacion=${selectedId}&theme=${theme}&panelId=10&refresh=1m&from=now-${totalHours}h&to=now`,
    total_generation: `${host_url}/02-generacion-jle?orgId=1&theme=${theme}&panelId=11&refresh=${refresh_time}&from=now-${totalHours}h&to=now`,
